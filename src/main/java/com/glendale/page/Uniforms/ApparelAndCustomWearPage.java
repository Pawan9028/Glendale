...
public class ApparelAndCustomWearPage {
	WebDriver driver;
	
	public ApparelAndCustomWearPage(WebDriver driver) {
		this.driver = driver;
	...
	@FindBy(xpath = "//div[@data-list-name='Category: Apparel & Custom Wear']/ul[@class='productGrid']/li")
	...
	@FindBy(xpath = "//tr/td/img") List<WebElement> iconsBelowTheTitle;

	public void methodToOpenTheApparelAndCustomWearPage() {
		Actions action = new Actions(driver);
		action.moveToElement(uniformTab).build().perform();
		uniformsList.get(5).click();
	}

	public void verifyPageURL() {
		methodToOpenTheApparelAndCustomWearPage();
		String currentURL = driver.getCurrentUrl();
		String expURL = "https://www.glendale.com/uniforms/apparel-custom-wear/";
	...
	public void verifyPageTitle() {
		...
		String expTitle = "Uniforms - Apparel & Custom Wear - Page 1 - Glendale Parade Store";
	...
	public void verifyTextPresenceAndFontSize() {
		...
		String expText = "Looking to show your team spirit or raise money ";
	...
	public void verifyProductNameIsPresence() {
		...
		public void verifyIconsBelowTheTitleIsVisible() {
			methodToOpenTheApparelAndCustomWearPage();
			for(int i=0;i<iconsBelowTheTitle.size();i++) {
				Assert.assertTrue(iconsBelowTheTitle.get(i).isDisplayed(),"Icon are not displayed.");
			}
			System.out.println("Icons below the title are displayd.");
		}
