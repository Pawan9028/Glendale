...
public class LetterOpenerSwordsPage {
	...
	private WebElement exploreCeremonialMilitarySwordsBtn;
	...
	public void verifyPageURL() {
		...
		String expURL = "https://www.glendale.com/swords/letter-openers/";
	...
	public void verifyProductImageSize() {
	 ...
	 //Responsive testing(Mobile) code
	 	...
	 	public void verifyPageLoadTime() {
	 		...
	 		System.out.println("Load Time for this page(Letter-Opener) is= "+totalTime);
	 	}
	 	...
	 	public void verifyExploreCeremonialMilitarySwordsButton() {
	 		methodToOpenTheLetterOpenerSwordsPage();
			Assert.assertTrue(exploreCeremonialMilitarySwordsBtn.isEnabled(),"Explore Letter Openers button is not enabled.");
			exploreCeremonialMilitarySwordsBtn.click();
			Assert.assertEquals(driver.getCurrentUrl(), "https://www.glendale.com/swords/ceremonial-military-swords/");
			System.out.println("Explore Cereminial military swords button is present and navigated to the Letter-Openers page.");
		}
		...
		public void verifyProductNameIsPresence() {
			...
			for(int i=0;i<priceOfTheProducts.size();i++) {
				Assert.assertTrue(priceOfTheProducts.get(4).isDisplayed(),"Price of the products is not displayed.");
			}
